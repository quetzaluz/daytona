...
class AsyncDaytona:
    ...
    async def get_or_create(
        ...
        *,
        ...
        Returns:
            AsyncSandbox: A Sandbox named ``params.name`` in the `started` state.
        ...
        Example:
            ...
            sandbox = await daytona.get_or_create(
        ...
        try:
            existing = await self.get(params.name)
        except DaytonaNotFoundError:
            existing = None

        if existing is not None:
            return await self._settle_existing_sandbox(existing, timeout=0)
    ...
    async def _settle_existing_sandbox(self, sandbox: AsyncSandbox, *, timeout: float) -> AsyncSandbox:
